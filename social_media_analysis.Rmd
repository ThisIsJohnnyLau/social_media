...
In this exercise, you will extract topics that are trending in Glasgow and also look at the most tweeted trends. The libraries rtweet and dplyr have been pre-loaded for you.
...
The most-tweeted trend in Glasgow is 'yeonjun' a Kpop star! This trend can be used as a theme for promoting a music-related event. 
...
It would be interesting to check the interest level and recall for ClimateCrisisby visualizing the frequency of tweets.
...
The time series plot shows low levels of tweet activity on ClimateCrisis in general with a few spikes that are not too significant.
...
100 XP
1
Create a time series object at hourly intervals for tweets on "#TEDxGlaClimate".

```{r}

# Create a time series object for TEDxGlaClimate_ts at hourly intervals
TEDxGlasgow_ts <- ts_data(twts_tedx, by ='hours')

# Rename the two columns in the time series object
names(TEDxGlasgow_ts) <- c("time", "TEDxGlasClimate_n")
...
merged_df <- merge(TEDxGlaClimate_ts, ClimateCrisis_ts, by = "time", all = TRUE)
head(merged_df)
...
ou can see that the final DTM has 4968 documents and 18088 terms. In the next exercise, you will use this DTM to perform topic modeling
